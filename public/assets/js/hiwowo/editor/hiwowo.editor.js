...
//(function ($) {
    ...
    Editor.prototype = Editor.fn = {
        config:{
            ...
            textareaID:"J_editorContent",
            toolbarId:"J_editorToolbar",
            ...
            btnImg:{
                ...
                exec:function (self) {
                    if (!self.ImgWrapDom || self.ImgWrapDom.length == 0) {
                        var html = '<div class="imgWrap">' +
                            '<form method="post" enctype="multipart/form-data" class="uploadImg clearfix" action="#" id="J_LocalImgForm">' +
                            '<input type="button" value="上传本地图片" class="bbl-btn upload-cover" id="J_swfUpload">' +
                            '<div class="clear"></div><span class="fl gc pt5 pl5">支持GIF、JPG、PNG,大小不超过2M</span>' +
                            '</form>' +
                            '<div class="netImg clearfix">' +
                            '<p>插入网络图片：</p>' +
                            '<input class="base-input" id="J_InsertNetImgInput" value="" placeholder="http://" autocomplete="off"/>' +
                            '<input type="button" id="J_InsertNetImgSubmit" class="bbl-btn" value="确定"></div>' +
                            '<div class="tipbox-up"><em>◆</em><span>◆</span></div>' +
                             '<div id="upload_bg" style=""> <div id="upload_color" style="width: 0px;"><div id="upload_percent">  <span id="upload_percent_num">0</span>% </div>  </div></div>'+
                            '</div>';
                        $('#' + self.config.toolbarId).append(html);
                        swfu = new SWFUpload({//全局变量
                            upload_url:"/editor/uploadPic",
