 ...
 */
 define(function(require){
     ...
     $.hiwowo.diagram={
         /* 判断节点是否可见 */
         isVisible : function(node) { return $(node).is(":visible"); },
         callbackFunc:function(func) { if(func) {func();} },
         // 点击可见的保存按钮
        clickBtn:function(node) {
         var visible = false;
         $(node).each(function() {
             if($.hiwowo.diagram.isVisible($(this))) {
                 $(this).click();
                 visible = true;
             }
         });
     ...
     buttonVisible:function(node) {
         ...
         getPreClassNode:function(node, className) {
         if($(node).prev().length > 0) {
             var preNode = $(node).prev();
             if($(preNode).hasClass(className)) {
                 return preNode;
             } else {
                 return $.hiwowo.diagram.getPreClassNode($(preNode), className);
             }
         }
         return null;
     },
         // 获取匹配到className的next元素
         getNextClassNode:function(node, className) {
         if($(node).next().length > 0) {
             var nextNode = $(node).next();
             if($(nextNode).hasClass(className)) {
                 return nextNode;
             } else {
                 return $.hiwowo.diagram.getNextClassNode($(nextNode), className);
             }
         }
         return null;
     },
         // 设置move bar的class
        checkAndSetMoveBar:function(node){
         var preNode = $.hiwowo.diagram.getPreClassNode($(node), "topic_node");
         var nextNode = $.hiwowo.diagram.getNextClassNode($(node), "topic_node");
       ...
       checkTitleNode:function(titleNode, length) {
        var msgNode = $(titleNode).closest("dl").find(".msg");
       ...
       insertImageNode:function(afterNode, image_id, image_url, image_intro) {
         ...
         clone.addClass("topic_node");
           $.hiwowo.diagram.checkAndSetMoveBar(clone);
         ...
         /* draft 参数的意义在于 判断是否是草稿保存，如果直接发布，则是发布到，这个需要带入到参数中，修改数据库中的字段 */
       saveDraft:function(draft) {
         var notSavedTopic = false;
         if($.diagram.clickBtn($(".img_save_href"))) {
             notSavedTopic = true;
         }
           if(notSavedTopic) {
               $.hiwowo.diagram.saveDraft(draft);
               return;
           }
         ...
         var diagramContent= $.hiwowo.diagram.getPicContents()
         var diagramPs =$("#J_ps").val()
         var diagramTags =$("#J_tags").val()
         var picIds = $.hiwowo.diagram.getPicIds()
         $.ajax({
     ...
     postDiagram:function() {
         if(!$.hiwowo.diagram.checkTitleNode($("#title"), 100)) {
             $("#title").focus();
             return false;
         }
         $("#id-btn-post").html("正在发布中");
         $.hiwowo.diagram.saveDraft(false);
     }
     ...
     $(function(){
         ...
         $(document).on("click",".move-up",function () {
                 ...
                 var preNode = $.hiwowo.diagram.getPreClassNode($(parentNode), "topic_node");
                 ...
                 $(".topic_node").each(function() {
                     $.hiwowo.diagram.checkAndSetMoveBar($(this));
                 });
         ...
         $(document).on("click",".move-down",function () {
                 var parentNode = $(this).closest(".topic_node");
                 var nextNode = $.hiwowo.diagram.getNextClassNode($(parentNode), "topic_node");
                 if(nextNode) {
         ...
         $(document).on("click",".img_save_href",function(){
             ...
             $.ajax({
                 ...
                 success: function(data) {
                     ...
                     if(data.code=="100") {
                         $.hiwowo.diagram.insertImageNode($(parentDiv), data.picId, picUrl, picIntro);
                         $(parentDiv).hide();
                     } else {
                        alert(data.message)
                     }
         ...
         $("#id-btn-draft").click(function(){
             $.hiwowo.diagram.saveDraft(true)
         })
         /* 发布 */
         $("#id-btn-diagram").click(function(){
             $.hiwowo.diagram.postDiagram()
         })
