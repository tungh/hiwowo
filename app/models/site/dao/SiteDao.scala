...
object SiteDao {
  ...
  /* add site */
  def addSite(uid:Long,title:String,pic:String,intro:String,tags:String) = database.withSession {  implicit session:Session =>
  ...
  def modifySite(sid:Long,title:String,pic:String,intro:String,tags:String) = database.withSession {  implicit session:Session =>
  ...
  /* 查找 site by id */
  def findSite(sid:Long) = database.withSession {  implicit session:Session =>
    ( for{
      s<- sites
      u<- users
    if s.uid === u.id
  ...
  /* 筛选 sites */
  def filterSites(uid:Option[Long],title:Option[String],status:Option[Int],startDate:Option[Timestamp],endDate:Option[Timestamp],currentPage:Int,pageSize:Int):Page[Site] =database.withSession {  implicit session:Session =>
