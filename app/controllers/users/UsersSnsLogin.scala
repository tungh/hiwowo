...
object UsersSnsLogin extends Controller {
  ...
  * */
  def snsLogin(snsType:String,backType:String,id:Long)=Action{ implicit  request=>
    if(snsType=="taobao"){
       Redirect("https://oauth.taobao.com/authorize?response_type=code&client_id=21136607&redirect_uri=http://hiwowo.com/user/taobao/registered/"+backType+"/"+id)
    }else if(snsType=="qzone"){
      Redirect(" https://graph.qq.com/oauth2.0/authorize?response_type=code&client_id=100344510&state=qq&redirect_uri=http://hiwowo.com/user/qzone/registered/"+backType+"/"+id)
    }else if (snsType=="sina"){
      Redirect("https://api.weibo.com/oauth2/authorize?client_id=1464940004&response_type=code&redirect_uri=http://hiwowo.com/user/sina/registered/"+backType+"/"+id)
    }
    else{
  ...
  def  registered(snsType:String,code:String,backType:String,i:Long)=Action{   implicit request =>
      ...
      * */
       ...
       val get = new HttpGet("https://graph.qq.com/oauth2.0/token?grant_type=authorization_code&client_id=100344510&client_secret=da726b47d8660a016d2e334ea27844fc&code="+code+"&redirect_uri=http://hiwowo.com/user/qzone/registered&state=qq")
       val client =new DefaultHttpClient();
   ...
   //    resp2.getAllHeaders.foreach(x=>println (x.getName  + " : " +x.getValue))
     ...
     /* 新浪微博登陆 */
       ...
       val get = new HttpPost("https://api.weibo.com/oauth2/access_token?client_id=1464940004&client_secret=52cb22ab45f9764a3b329578c70b89df&grant_type=authorization_code&redirect_uri=http://hiwowo.com/user/sina/registered&state=sina&code="+code)
       val client =new DefaultHttpClient();
       val resp= client.execute(get)
       val entity=resp.getEntity;
       val r=EntityUtils.toString(entity)
